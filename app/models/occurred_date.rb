class OccurredDate < ApplicationRecord
  ...
  private
    ...
    def update_event_after_save
      self.event.update(occurred_at: self.event.occurred_dates.first.occurred_at)
    end

    def update_event_before_destroy
      self.event.update(occurred_at: self.event.occurred_dates.where.not(id: self.id).first.occurred_at)
    end
end
